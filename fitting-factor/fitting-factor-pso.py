...
ecc_list = np.linspace(1e-6, 0.0012, 11)
...
for i in range(iter):
    ...
    #scipy.optimize(objective_match, method='nelder-mead')
    optimizer = GlobalBestPSO(n_particles=n_particles, dimensions=2, options=PSO_OPTIONS, bounds=bounds, init_pos=init_pos)

    log_mismatch, rec_prms = optimizer.optimize(objective_wrapper, iters=1, injection_1 = injection_channel_1, injection_2 = injection_channel_2, **objective_kwargs)
...
label = 'pso-maximise-'+ str(ecc_list[inj_arg]).replace('.', 'p') + '.pkl'
